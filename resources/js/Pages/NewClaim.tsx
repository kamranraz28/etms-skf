...
export default function NewClaim({ vendor }: any) {
  ...
  const submit = () => {
    if (!tenderNumber || !title || !amount || documents.every((d) => !d.file))
      return;
    setSubmitting(true);
  ...
  return (
    <AppShell>
      ...
      <PageHeader
        title="New Claim"
        description="Submit a billing claim with supporting documents."
      />
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="panel p-5 space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Brief title for this claim"
              />
            </div>
            <div>
        ...
        <div className="panel p-4 h-fit space-y-3">
          ...
          <div className="text-xs">
            ERP: <span className="font-mono">{vendor.erp_code ?? "—"}</span>
          </div>
          <div className="text-xs text-muted-foreground leading-relaxed pt-2 border-t border-border">
            Your claim will be reviewed by the procurement panel, then approver
            panel, then admin for final approval. You can track the lifecycle
            from My Claims.
          </div>
        </div>
      </div>
