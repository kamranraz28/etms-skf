...
import { ArrowLeft, Download, Lock, Scale, ExternalLink } from "lucide-react";
...
export default function TenderShow({ tender, vendors, bids, cs }: any) {
  ...
  return (
    <AppShell>
      ...
      <PageHeader title={tender.title} description={`${tender.tender_number} · Deadline ${new Date(tender.deadline).toLocaleString()}`}
        actions={
          <div className="flex gap-2 items-center">
            <StatusBadge status={tender.status} />
            {tender.status === "open" && <Button variant="outline" size="sm" onClick={closeTender}><Lock className="h-4 w-4 mr-1" /> Close tender</Button>}
            ...
            {cs && <Link href={`/app/cs/${cs.id}`}><Button size="sm" variant="outline"><ExternalLink className="h-4 w-4 mr-1" /> Open CS</Button></Link>}
          </div>
        } />
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="panel">
            ...
            <table className="data-table">
              <thead><tr><th>Vendor</th><th>ERP</th><th className="text-right">Total price</th><th>Currency</th><th>Submitted</th><th>Document</th></tr></thead>
              <tbody>
                {bids.length === 0 && <tr><td colSpan={6} className="text-center text-muted-foreground py-6">No bids yet.</td></tr>}
                {bids.map((b: any) => (
                  <tr key={b.id} className={lowest?.id === b.id ? "bg-success/5" : ""}>
                    <td className="font-medium">{b.vendor?.name}{lowest?.id === b.id && <span className="ml-2 text-[10px] uppercase tracking-wider text-success">L1</span>}</td>
                    <td className="font-mono text-xs">{b.vendor?.erp_code ?? <span className="text-warning">—</span>}</td>
                    ...
                    <td>{b.document_path ? <a href={`/app/bids/${b.id}/document`}><Button size="sm" variant="ghost"><Download className="h-3.5 w-3.5 mr-1" /> Open</Button></a> : <span className="text-xs text-muted-foreground">—</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {(tender.pr.items ?? []).map((it: any, i: number) => (
                    <tr key={i}><td>{it.name}</td><td>{it.qty}</td><td>{it.unit}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
