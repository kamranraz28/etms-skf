    ...
    <AppShell>
      ...
      <div className="panel p-4 mb-5">
        <div className="flex items-end gap-3">
          ...
          <Button onClick={filter}><Search className="h-4 w-4 mr-1" /> Filter</Button>
        </div>
      </div>
      <div className="panel">
          ...
          <tbody>
            ...
            {claims.map((c: any) => (
              <tr key={c.id}>
                ...
                <td>{c.vendor?.name}</td>
                <td className="font-mono text-xs">{c.vendor?.erp_code ?? "—"}</td>
                <td className="font-mono text-xs">{c.tender_number}</td>
                <td className="max-w-40 truncate">{c.title}</td>
                <td className="text-right font-mono">{Number(c.amount).toLocaleString()}</td>
                <td className="text-xs text-muted-foreground">{new Date(c.submitted_at).toLocaleString()}</td>
                <td><StatusBadge status={c.status} /></td>
                <td className="text-right"><Link href={`/app/claims/${c.id}`}><Button size="sm" variant="outline">View <ChevronRight className="h-3.5 w-3.5 ml-0.5" /></Button></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
