    ...
    <AppShell>
      ...
      <div className="panel">
        <table className="data-table">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={6} className="text-center text-muted-foreground py-8">No tenders assigned yet. Make sure your vendor profile is approved.</td></tr>}
            {rows.map((t: any) => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td className="text-xs">{new Date(t.deadline).toLocaleString()}</td>
                  <td><StatusBadge status={t.status} /></td>
                  <td>{t.hasBid ? <span className="text-success text-xs">Submitted</span> : <span className="text-muted-foreground text-xs">—</span>}</td>
