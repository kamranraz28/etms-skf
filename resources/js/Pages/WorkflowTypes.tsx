...
export default function WorkflowTypes({ types, roles }: any) {
  ...
  const roleOptions: any[] = roles?.length ? roles : [
    { value: "approver", label: "Approver" },
    { value: "department_head", label: "Department head" },
    { value: "executive_director", label: "Executive director" },
    { value: "scm_head", label: "SCM head" },
    { value: "finance_head", label: "Finance head" },
    { value: "admin", label: "Admin" },
  ];

  const labelFor = (role: string) => roleOptions.find((r: any) => r.value === role)?.label
    ?? role.split("_").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");

  const setStep = (i: number, role: string) => {
    const copy = [...steps];
    copy[i] = { role_name: role, step_name: role, label: role ? labelFor(role) : "" };
    setSteps(copy);
  };
  ...
  const save = () => {
    ...
    for (const s of steps) {
      if (!s.role_name) {
        sa.alert("Error", "Select an approval level for every step", "error");
        return;
      }
    }
                ...
                <div className="space-y-3">
                  {steps.map((s, i) => (
                    <div key={i} className="flex gap-3 items-center p-4 rounded-xl border border-border/60 bg-gradient-to-br from-card to-muted/15 relative">
                      ...
                      <div className="flex-1 space-y-1.5">
                        ...
                        >
                          ...
                          {roleOptions.map((r: any) => (
                            <option key={r.value} value={r.value} disabled={steps.some((x, xi) => xi !== i && x.role_name === r.value)}>
                              {r.label}
                            </option>
                          ))}
                        </select>
                      ...
                      {steps.length > 1 && (
                        <Button size="sm" variant="ghost" onClick={() => removeStep(i)} className="h-8 w-8 p-0 shrink-0 hover:bg-destructive/10">
                          <Trash2 className="h-4 w-4 text-destructive" />
                        </Button>
                      )}
                    </div>
