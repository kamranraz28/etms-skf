...
import { ExternalLink, Scale, ArrowUpRight, Workflow } from "lucide-react";
      ...
      <div className="card-grid stagger-children">
        ...
        {rows.map((cs: any) => (
          <Link key={cs.id} href={`/app/cs/${cs.id}`} className="group relative bg-card border border-border/60 rounded-2xl p-5 hover-lift overflow-hidden">
            ...
            {cs.workflow_type && (
              <div className="flex items-center gap-1 text-[10px] text-accent mt-1.5">
                <Workflow className="h-3 w-3" /> {cs.workflow_type.name}
              </div>
            )}
