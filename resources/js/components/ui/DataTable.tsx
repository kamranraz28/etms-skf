...
import {
  ArrowUpDown, ArrowUp, ArrowDown, Search, Download, FileSpreadsheet,
  FileText as FileTextIcon, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  File as FilePdf, SlidersHorizontal, X, FileDown,
} from "lucide-react";
    ...
    <tr className="animate-pulse">
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="px-4 py-3">
          <div className={cn("h-4 rounded-lg bg-muted/60", i === 0 ? "w-3/4" : "w-full")} />
        </td>
      ))}
    </tr>
  );
...
export function DataTable<T extends Record<string, any>>({
  ...
  const exportCSV = async () => {
    ...
    const body = sorted.map((row) =>
      exportCols.map((c) => `"${String(row[c.key] ?? "").replace(/"/g, '""')}"`).join(","),
    ).join("\n");
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...
        headStyles: { fillColor: [41, 65, 102], textColor: 255, fontStyle: "bold" },
  ...
  const SortIcon = ({ col }: { col: Column<T> }) => {
    ...
    if (sortKey !== col.key) return <ArrowUpDown className="h-3 w-3 ml-1.5 opacity-30 group-hover:opacity-60 transition-opacity" />;
    return sortDir === "asc"
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 w-full sm:w-auto">
          ...
          {filterable && (
            <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)}
              className={cn("transition-all duration-200", showFilters && "bg-accent/10 border-accent/30 text-accent")}>
            ...
            <div className="relative" ref={exportRef}>
              ...
              {exportOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-44 bg-card border border-border/60 rounded-xl shadow-xl z-50 py-1.5 animate-scale-in origin-top-right">
                  <button onClick={exportCSV} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground/80 hover:bg-muted/50 transition-colors">
                    <Download className="h-4 w-4 text-muted-foreground" /> Export as CSV
                  </button>
                  <button onClick={exportExcel} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground/80 hover:bg-muted/50 transition-colors">
                    <FileSpreadsheet className="h-4 w-4 text-success" /> Export as Excel
                  </button>
                  <button onClick={exportPDF} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground/80 hover:bg-muted/50 transition-colors">
                    <FilePdf className="h-4 w-4 text-destructive" /> Export as PDF
                  </button>
                </div>
              )}
            </div>
                ...
                {columns.map((col) => (
                  <td key={col.key} className={cn(
                    ...
                    {col.render ? col.render(row) : (
                      <span className={row[col.key] != null ? undefined : "text-muted-foreground/60"}>
                        {row[col.key] ?? "—"}
                      </span>
                    )}
                  </td>
